...
public class NServiceImpl implements NService
{
	 ...
	 */	  
	public boolean userLogin(String username, String password)throws NException
	{
		try
		{
			User user = ud.findByNameAndPass(username, password);
			if (user ==null)
			{
				return false;
			}
	 ...
	 * @return 该用户对应的相册列表。
	 */
	public List<AlbumVO> getUserAlbums(int userId , int first, int pageSize)throws NException
	{
		try
		{
			User user = ud.get(userId);
	...
	/**
	 * 根据种类来获取相册列表
	 * @param ChannelId 需要获取的相册种类ID
	 * @param first 需要显示的第一个相册
	 * @param pageSize 每页显示的相册数量
	 * @return 查询到的相册列表
	 */
	public List<AlbumVO> getAlbumsByChannel(Integer channelId, int first, int pageSize)throws NException
	{
		try
		{
			Channel channel = cd.get(channelId);
	 ...
	 */
	public List<PhotoVO> getPhotos(Integer albumId, int first, int pageCount)throws NException
	{
		try
		{
			Album album = ad.get(albumId);
		...
		Date date, boolean cover, Integer albumId)throws NException
	{
		...
		{
			Album album = ad.get(albumId);
			Photo photo = new Photo(title, keyword, times, picUrl, bigPicUrl, smallPicUrl, date,cover, album);
			pd.save(photo);
		}
		catch (Exception e)
		{
	 ...
	 * 修改相片信息
	 */
	public void updatePhoto(String title, String keyword, Integer albumId, Integer photoId)throws NException
	{
		try
		{
			Photo photo = pd.get(photoId);
	...
	{
		UserVO uvo = new UserVO(user.getId(), user.getUsername(), user.getPassword());
		return uvo;
	}
	...
	{
		PhotoVO pvo = new PhotoVO(photo.getId(), photo.getTitle(), photo.getKeyword(), photo.getTimes(), 
			photo.getPicUrl(), photo.getBigPicUrl(), photo.getSmallPicUrl(), photo.getDate(), 
			photo.isCover(), photo.getAlbum().getId(), photo.getAlbum().getTitle());
		return pvo;
	}
